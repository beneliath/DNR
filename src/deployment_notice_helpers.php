...
function deploymentNoticeStatus(?string $path = null, ?float $now = null): array
{
    ...
    if (!is_array($notice)
        ...
        || !in_array($notice['phase'] ?? null, ['pending', 'deploying', 'failed'], true)
    ) {
        return $payload;
    }
    foreach (['started_at', 'not_before', 'expires_at'] as $field) {
        if (!is_int($notice[$field] ?? null) && !is_float($notice[$field] ?? null)) {
            return $payload;
        }
    }
    if ($notice['not_before'] < $notice['started_at'] + 300
        || ($notice['phase'] === 'pending' && $notice['expires_at'] <= $now)
    ) {
        return $payload;
    }
    ...
    $payload['notice'] = array_intersect_key($notice, array_flip([
        'id', 'phase', 'started_at', 'not_before', 'expires_at',
    ]));
    return $payload;
}
