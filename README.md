...
HTTP entry points load `src/bootstrap.php`, which initializes Composer/local autoloading, structured request logging, database configuration, and shared application helpers. Reusable validation and reference data live under `src/app/Domain`; create and edit routes use the same normalizers. PHPStan checks the complete HTTP surface at level 0, the legacy helper layer at level 4, and the typed domain/runtime layer at level 6 so newly extracted code can move progressively into the stricter configurations.
