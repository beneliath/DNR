-- Minimal pre-migration schema. Every subsequent schema change belongs in a
-- forward migration so fresh installs and upgrades follow the same path.
...
CREATE TABLE engagements (
    ...
    compensation_type ENUM(
        'Unknown', 'Honorarium', 'Offering',
        'Honorarium and Offering', 'Other'
    ) DEFAULT 'Unknown',
    ...
    is_deleted TINYINT(1) DEFAULT 0,
    CONSTRAINT fk_baseline_engagement_organization
...
CREATE TABLE presentations (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_baseline_presentation_engagement
...
CREATE TABLE contacts (
    ...
    CONSTRAINT fk_baseline_contact_organization
        FOREIGN KEY (organization_id) REFERENCES organizations(id)
);
