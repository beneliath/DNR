...
import argparse, copy, hashlib, html, json, re, shutil, unicodedata
...
EDITION_DATE='September 24, 2026'
...
ROUTE_CH={
 ...
 'ai_coach_requests.php':12,'ai_coach_improvements.php':12,
}
...
class Cover(Flowable):
    ...
    def draw(self):
        ...
        c.setFillColor(MUTED);c.setFont('Body',9);c.drawString(30,143,f'Application version {VERSION}  |  {EDITION_DATE}')
        c.drawString(30,124,'Comprehensive Guide for this application release')
        c.drawString(30,106,'Reviewer, editor, and administrator reference')
...
class ManualDoc(BaseDocTemplate):
    def __init__(self,path,known=None):
        super().__init__(str(path),pagesize=(W,H),leftMargin=M,rightMargin=M,topMargin=50,bottomMargin=44,title='MOED Comprehensive User Manual',author='MOED / DNR',subject=f'Illustrated application reference, version {VERSION}, updated {EDITION_DATE}',pageCompression=1)
    ...
    if e.tag in ('ul','ol'):
        for n,li in enumerate(e.findall('./li'),1):
            if e.tag=='ol':
                ...
                # Separate block paragraphs from the step's lead-in without
                # inserting spaces before punctuation after inline emphasis.
                piece=copy.deepcopy(pieces[0] if pieces else li)
                for block in piece.xpath('./p|./section'):
                    previous=block.getprevious()
                    if previous is not None:
                        previous.tail=(previous.tail or '').rstrip()+' '
                content=inline(piece)
...
SUPPLEMENTS={
 ...
 8:[('Email delivery states and retries',['Queued messages can be Pending or Processing before becoming Sent, Failed, Delivery Uncertain, or partially delivered across recipients. Open the delivery record to inspect individual recipient outcomes and timestamps.','Retry Failed Deliveries retries the failures rather than re-sending successful recipients. Correct a bad destination before retrying. Inquiry delivery retries are available while the inquiry is active. Delivery Uncertain means the message may already have arrived. Check the provider reference or recipient first; only then acknowledge the review and use Retry Reviewed Deliveries.','After booking conversion, replies to earlier inquiry email route into the resulting engagement. The original inquiry retains the pre-booking record and any tasks not moved.','The Inbox filing panel saves an email to existing destinations. Create a new booking request from New Inquiry in Booking Pipeline when needed, then return to the Inbox and review the appropriate destinations.'])],
...
SUPPLEMENTS[4]=[('Archive a finished inquiry',[
    'Open a declined inquiry or an inquiry that was booked and converted to an engagement. Select Archive Inquiry in the Archive This Inquiry panel or on its pipeline card.',
    'Archiving retains the stage, request details, activity, correspondence, tasks, and any surviving engagement link. It removes the inquiry from ordinary pipeline views, counts, and exports, and hides its tasks from active work until restored. It does not archive or change the engagement.',
    'Choose Archived Inquiries to search retained records or export that view. Open an archived inquiry to read its information, or select Restore Inquiry to return it to its original Booked or Declined stage.',
    'Restore a declined inquiry before reopening it to Qualified. A booked inquiry remains a read-only source record after restoration. A recorded conversion still permits archiving when its engagement was later deleted; an inquiry merely labeled Booked without a recorded conversion is not eligible.'
])]
SUPPLEMENTS[7]=[('Archive tasks without changing their status',[
    'To archive a finished item, choose Completed or canceled in the Work Queue, then Archive task.',
    'In Archived, review both badges. Restore task retains details, status, owner, and due date; completed work stays completed. An open overdue task becomes eligible for reminders again when its engagement or inquiry is also unarchived.'
])]
SUPPLEMENTS[8].append(('Read mail and historical mailbox review',[
    'MOED tracks new mailbox messages independently of read or unread flags. Reading a message in your mail client does not prevent its import, and importing does not alter the original message or its read state.',
    'The first mailbox scan, or recovery after a mailbox identity reset, can hold unfamiliar historical messages for operator review. Marking an older message unread does not request another import.',
    'Ask the deployment operator to review pending historical mail and import or skip it deliberately. The README appendix contains the reconciliation commands. Use Inbox to route a message after it has been imported.'
]))
SUPPLEMENTS[12].append(('Understand mailbox health',[
    'Mail Ingestion separates the most recent mailbox check from the most recent new import. A current check with no new import can simply mean that no new mail arrived.',
    'A failed or overdue check, historical review backlog, or quarantined message needs operator attention. These are separate from messages already in Inbox waiting for a user to select the correct Chron logs.'
]))
...
SUPPLEMENTS[1].append(('Use Enter without losing your place',[
    'In the related-record search on a task or the engagement search in the Inbox, Enter runs the search. It does not save the task or file the message. Review the results, choose the intended destination, then use the explicit Save action.',
    'Inquiry organization/contact searches and the engagement contact picker also use Enter to search. Use Tab to move between controls; Enter on a focused button activates that button.',
    'In the bulk-user deletion review, type DELETE USERS and press Enter or select Permanently delete. The Are you sure? dialog still requires confirmation. Cancel closes the dialog without deleting; Cancel on the review page abandons the selection.',
    'Financial Closeout offers Save draft before Finalize. Enter in an amount field saves a draft; use the explicit finalization control after reviewing the required completion conditions.'
]))
SUPPLEMENTS[5][0][1].insert(2, 'Selecting a replacement PDF or PowerPoint, or checking Remove, is pending until you select Save Changes in the persistent bar at the bottom of the Edit Engagement window. This saves the entire engagement form. The notice beside the file identifies the pending change. Choose replacement or removal, not both. Cancel or leave without saving to keep the current file; after saving, check the updated filename and upload details.')
SUPPLEMENTS[6].append(('Select records for permanent deletion',[
    "Administrators can use a row's individual Delete action or select checkboxes on Organizations, Contacts, Speakers, Engagements, Tasks, or Users and choose Delete selected. Editors and reviewers cannot delete records or access the selection controls.",
    'The selected count and Select all on this page apply only to displayed rows, up to 100. Clear selection resets the checkboxes. Changing pages or filters starts a new selection, so finish one page before continuing to another.',
    'Review the selected names and related data before confirming. Organization deletion can remove engagements and contacts that have no other organization; shared contacts are kept. Contact deletion removes affiliations, roles, tasks, and Chron. Engagement deletion removes its presentations, files, tasks, financial records, statistics, and Chron.',
    'Unused speakers can be deleted individually or in a group. Presentations, short links, notes, or slide decks protect a referenced speaker. Tasks are deleted without deleting their related record or standard task definition.',
    'Complete Admin Unlock if requested. The selection survives the redirect and expires after 30 minutes. The server rechecks the five-minute unlock at deletion time. After the Are you sure? dialog, the list reports how many selected records were deleted and how many were kept or failed. Review remaining items before retrying.'
]))
SUPPLEMENTS[12].append(('Delete inactive users individually or in a group',[
    'On each user card, editing and security actions are grouped together; activation and deletion controls are grouped separately. Amber Deactivate suspends access. Red Delete user permanently removes the account. Active accounts and the account you are using cannot be deleted.',
    'To suspend an active account, unlock administrator actions, select Deactivate, and confirm. The existing five-minute countdown continues from its original deadline; it does not restart or end early. Deletion also preserves the same deadline.',
    'For one inactive or invited account, select Delete user, unlock if prompted, and type DELETE USER in the Are you sure? confirmation. Review the account name before submitting.',
    'For several accounts, select their checkboxes, choose Delete selected, and review the names. Type DELETE USERS, then select Permanently delete or press Enter to open the final Are you sure? dialog. Cancel leaves the accounts in place. Confirm only when permanent removal is intended.',
    'Active accounts and your own account have no deletion checkbox and remain protected on the server. A failed or newly protected item is kept while other eligible selections may be deleted. Read the result counts before repeating the action.'
]))
...
SUPPLEMENTS[12].append(('Backups include uploaded files',[
    'PDF Speaker Notes, PPT Slidedecks, and profile, contact, and speaker photos are saved in persistent file storage. Application records retain their metadata and associations. Uploading and replacing them uses the same forms as before.',
    'The encrypted .dnrbackup export contains both the database and the uploaded files needed by those records. Keep the whole backup together and give the deployment operator the matching backup password and separately protected 2FA encryption key when arranging recovery.',
    'Restoring an older database-only backup cannot recover files it never contained. The operator should follow the current README restore procedure and verify the restored attachments before reopening the application.'
]))

TOPICS=[
 ('Customize the shared standard event checklist','topic-13-every-new-event-includes-tasks-we-do-not-need'),('Missing presentation file or QR code','topic-13-a-presentation-file-or-its-qr-code-is-missing'),
 ('PPT Slidedeck: upload, replace, and download','shot-presentation-files'),('Copy a QR link for sharing','shot-presentations'),('Due tomorrow task labels','shot-tasks'),('Uploaded files and encrypted backups','topic-12-backups-include-uploaded-files'),
 ('Archive and restore inquiries','shot-inquiry-archive'),('Archived inquiry search and export','shot-inquiry-archived'),('Archive and restore tasks','shot-task-archived'),('Daily digest delivery days on user cards','shot-users'),('Engagement lifecycle filter memory','shot-engagement-list'),('Historical mailbox review','topic-8-read-mail-and-historical-mailbox-review'),('Lock administrator actions now','shot-admin-countdown'),('Mail ingestion health','shot-operations'),
...
class ScreenshotRegion(Flowable):
    ...
    def __init__(self,path,size,region,scale,omitted_regions=None):
        ...
        if not (0<=self.x and 0<=self.y and self.region_width>0 and self.region_height>0
    ...
    def draw(self):
        ...
        for omitted in self.omitted_regions:
            ...
            # Underpaint past the clipping edge to prevent a white antialias seam.
            canvas.rect(px-.5,py-.5,w*self.scale+1,h*self.scale+1,stroke=0,fill=1)
...
TOPICS += [
    ('ai coach: ask and follow up', 'topic-1-learn-with-ai-coach'),
    ('ai coach: interactive walkthroughs', 'topic-1-follow-an-interactive-walkthrough'),
    ('ai coach: stop or start a conversation', 'topic-1-manage-the-coach-conversation'),
    ('ai coach: helpful, needs work, and missing controls', 'topic-1-rate-an-answer-and-report-a-missing-control'),
    ('ai coach: request history and assessment', 'topic-12-review-ai-coach-requests'),
    ('ai coach: verified guidance and feedback review', 'topic-12-turn-coach-feedback-into-verified-improvements'),
    ('ai coach: clear the request log', 'topic-12-clear-the-ai-coach-request-log'),
    ('ai coach: slow or unavailable answers', 'topic-13-ai-coach-is-slow-or-cannot-answer'),
]
...
def story(known):
    ...
    out.extend([Spacer(1,14),heading('Edition and source notes','edition-notes',0,'h3'),p(f'Application version: MOED / DNR {VERSION}. Guide updated: {EDITION_DATE}. This refresh adds continuous desktop calendar scrolling, complete-month navigation, individual ai coach request deletion, numbered presentation sections, and PDF/PowerPoint drag-and-drop uploads. It clarifies the recovery-email form and retains existing guidance for administrator unlock, invitations, and network comparisons, whose recent layout changes do not alter their workflows. It also retains ai coach conversations, navigation during an answer, walkthrough controls, feedback, administrator request history, verified improvement cases, and request-log clearing. It clarifies the feedback investigation cycle and the limits of automatic learning. PowerPoint uploads now allow 500 MB per file, PDF speaker notes allow 100 MB, and the engagement editor transfers them in small chunks with visible progress and automatic retries before saving. Event creation and presentation uploads name their actual pages and tabs. The guide retains shared standard-checklist guidance, clickable record rows and cards, persistent Edit Engagement actions, and gold financial-closeout and purple mail-review indicators. It retains the 2.2.3 coverage of protected bulk deletion and administrator access. Presentation-file instructions now use the persistent Save Changes bar. Online manual search now lists and highlights specific topics, opens matching troubleshooting entries, and provides fixed Top and Bottom navigation. The ai coach topics provide numbered instructions verified against the current application. Existing illustrations use fictional records from an isolated preview and are retained where their controls still apply; per-figure metadata records capture dates. Source basis: src/help.php, application pages/forms/controllers, and the Mattermost integration guide.'),p('The PDF follows the sidebar guide, including private-calendar content options, birthdays, recovery-email verification, account editing, and network diagnostics. The PDF adds illustrated walkthroughs and operational details, omitting decorative application footers and their associated lines. Infrastructure-only procedures are referenced at the point where a deployment operator is required.'),p('Document build and screenshots: scripts/manual/ and docs/user-manual/. Rebuild with the included instructions when the application changes. Navigation is internal to this PDF; links do not depend on the sample preview being available.'),p('<link href="#contents" color="#2457d6">Return to contents</link>')])
...
def main():
    ...
    report={'version':VERSION,'edition_date':EDITION_DATE,'pages':len(reader.pages),'screenshots':len(SHOTS),'link_annotations':links,'destinations':known,'outline_entries':len(doc.entries),'edition_scope':f'Application release {VERSION}', 'sources':['src/bulk_delete.php','src/bulk_delete_helpers.php','src/delete_user.php','src/user_lifecycle.php','src/assets/js/task-form.js','src/assets/js/inbound-mail.js','src/assets/js/footer.js','src/presentation_slidedeck_helpers.php','src/persistent_file_helpers.php','src/templates/presentation_short_links.php','src/help.php','src/templates/ai_coach.php','src/assets/js/ai-coach.js','src/ai_coach_requests.php','src/ai_coach_improvements.php','src/ai_coach_feedback_helpers.php','src/ai_coach_improvement_helpers.php','src/view_calendar.php','src/profile.php','src/map_pin.php','src/templates/presentation_form.php','src/network_diagnostics.php','src/network_diagnostics_helpers.php','src/edit_user.php','src/database_maintenance.php','src/daily_digest_email.php','src/tasks.php','src/templates/header.php','src/inquiries.php','src/view_inquiry.php','src/booking_inquiry_helpers.php','src/users.php','src/operations.php','src/inbound_ingestion_helpers.php','src/task_visibility_helpers.php']}
