...
EDITION_DATE='September 15, 2026'
...
def story(known):
    ...
    out.extend([Spacer(1,14),heading('Edition and source notes','edition-notes',0,'h3'),p(f'Application source: MOED / DNR {VERSION}. Guide updated: {EDITION_DATE}. Inquiry archive/restore, task archive/restore, engagement filters, user digest cards, administrator unlock/lock, and mail ingestion illustrations were refreshed for this edition. This release includes inquiry archiving and hides unfinished tasks while their engagement or inquiry is archived. Mailbox health is a synthetic example. September 14 navigation, network, backup, profile, and Dashboard captures are retained where unchanged; network timings and Dashboard records are synthetic examples. Presentation and scheduling captures from September 13, inbox/template captures from September 10, and other September 9 views are retained where unchanged. Per-figure metadata records refreshed captures. Source basis: src/help.php, application page/forms/controllers, and the Mattermost integration guide.'),p('The PDF follows the sidebar guide, including private-calendar content options, birthdays, recovery-email verification, account editing, and network diagnostics. The PDF adds illustrated walkthroughs and operational details, omitting decorative application footers and their associated lines. Infrastructure-only procedures are referenced at the point where a deployment operator is required.'),p('Document build and screenshots: scripts/manual/ and docs/user-manual/. Rebuild with the included instructions when the application changes. Navigation is internal to this PDF; links do not depend on the sample preview being available.'),p('<link href="#contents" color="#2457d6">Return to contents</link>')])
