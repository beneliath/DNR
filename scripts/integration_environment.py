...
def main():
    if sys.argv[1:] == ['verify']:
        ...
        return
    if sys.argv[1:] not in ([], ['downloads']):
        raise ValueError('Usage: integration_environment.py [downloads|verify]')
    ...
    with tempfile.TemporaryDirectory(prefix='dnr-test-') as temporary:
        ...
        used = subprocess.check_output(['docker', 'network', 'inspect', *networks], text=True) if networks else ''
        for _ in range(100):
            octet = 20 + secrets.randbelow(220)
            subnet = f'10.252.{octet}.0/24'
            if subnet not in used:
                values.update(DNR_BACKEND_SUBNET=subnet, DNR_INGRESS_PROXY_IP=f'10.252.{octet}.254')
                break
        else:
            raise ValueError('Unable to allocate a test network')
        ...
        services = ('web', 'downloads', 'file-monitor', 'file-migrator', 'backup', 'ingress', 'geocoder', 'maintenance', 'db', 'migrator', 'mail-ingest')
