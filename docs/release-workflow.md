...
## Comprehensive Guide version for s1

Whenever an s1 deployment is requested, check whether the Comprehensive Guide PDF has changed for that deployment, comparing the release contents with the last successfully deployed s1 revision. Include guide changes already merged into `main` and pending guide source changes that require a rebuild.

If the guide has changed, its application-version reference must identify the app version it applies to in that deployment. Finalize the release `VERSION`, then rebuild, inspect, verify, and install `src/assets/docs/moed-comprehensive-user-manual.pdf` following the [manual build instructions](user-manual/README.md). The builder reads the root `VERSION`; verify that the finished PDF's cover, scope/edition notes, appendix version reference, and metadata agree. Update stale draft wording about local changes since an earlier production release while keeping clearly identified historical references accurate.

Commit the verified PDF and any updated guide sources/reports with the release before merge, final-main CI, and image publication. If the release version changes again, repeat the guide rebuild and verification. This requirement applies to every requested s1 deployment, including all deployment shorthands below; an unchanged guide does not require a version-only refresh. See the persistent [Comprehensive Guide version rule](../.cursor/rules/s1-comprehensive-guide-version.mdc).
...
This publishes a preparation notice on s1 at `192.168.1.150` and remembers the workflow ID in the local Git directory. It immediately says **“An update is being prepared”** and tells users they can continue working. No countdown runs during version preparation, protected merge, final-main CI, remote verification, or image downloads. Repeating `start` from the same checkout preserves its current stage and any countdown already started. Another active workflow cannot replace it.

Authenticated application pages display a persistent red banner with white text above the content, stacked with the View As banner. The background uses a stronger red in both light and dark themes. It polls every five seconds without opening or extending a login session. After validation succeeds and s1 finishes downloading and verifying all release images, the server starts one shared five-minute countdown. Users see `5:00` counting down based on server time; polling can make the first visible tick a few seconds lower. New pages/tabs get the remaining time, not a new five-minute window. At zero, the message becomes **“Update pending — please save your work. Deployment will begin shortly.”** The application remains usable during preparation. Only when s1 starts maintenance does it say **“Deployment in progress.”** Forms are never automatically reloaded, submitted, or cleared by the banner.

After image pulls and preflight, the s1 command atomically changes the notice from `preparing` to `pending`, records `countdown_started_at`, binds the countdown to the qualified commit, and sets `not_before` to exactly 300 seconds later. It enforces that deadline before stopping writers and taking the verified backup. Three minutes or thirty minutes of preparation both result in a full five-minute save window. Retrying the same countdown waits only its remaining time; it cannot reset the deadline or substitute another commit. A pending notice created by the previous workflow retains its original deadline for compatibility. The timer reaching zero never bypasses validation or backup requirements. The public ingress serves the read-only notice independently of the web/database containers, keeping it available during maintenance.

`./scripts/deploy_s1.sh EXPECTED_MAIN_SHA` automatically reuses the remembered notice. If invoked directly without a notice, it starts one and enforces the full remaining save window. When continuing from a different checkout, set `DNR_DEPLOYMENT_NOTICE_ID` to the ID printed by `start` (or pass it as the second argument to the notice command). IDs survive ambiguous SSH failures so retrying cannot reset the preparation stage or an existing timer.
...
Use `./scripts/deployment_notice.sh status` to inspect the remote state. The s1 command automatically cancels a preparation or pending notice when its preflight fails and clears a completed notice after success. Once maintenance begins, cancellation cannot clear it: a failed deployment displays **“Update needs attention”** and requires the recovery procedure below. After recovery has been verified, run `./scripts/deployment_notice.sh resolve NOTICE_ID` to clear the notice; this refuses while a deployment still holds the host lock. Do not use resolution to bypass recovery.

An abandoned preparation notice expires after six hours. Starting the countdown gives its pending notice a fresh six-hour expiry so a long preparation cannot truncate the save window. Expiration clears its banner but also makes it ineligible for deployment; cancel it and start a new workflow to provide a fresh preparation notice and save window. Failed or active maintenance notices do not expire silently. The state is stored in the ignored `var/deployment/` directory, mounted read-only into web and ingress; only SSH deployment tooling writes it. The public status endpoint exposes only the global notice and timing, with no user, credential, source SHA, or recovery information.
...
3. Apply the [Comprehensive Guide version requirement](#comprehensive-guide-version-for-s1) after finalizing the release version. Commit all pending project changes, including the version bump and any rebuilt guide, on the current working branch before merging. Publish the working branch as needed for the protected PR workflow.
...
- For releases with persistent files, also archives the private uploaded-files volume, checks every registered file against its database size/checksum, encrypts the companion archive, and verifies its decryption round trip. The receipt records both archives; keep them together.
...
- After SQL migrations, converts legacy PDFs/portraits into persistent files with the resumable `file-migrator`, then performs any initial speaker import. After that, renders and stores missing QR images using the public MOED origin while writers remain paused. Existing rendered images are preserved. A failed backfill leaves writers paused with the verified recovery backup available.
...
Native database archives use the `.sql.gz.dnrenc` suffix. When `receipt.json` contains `persistent_files`, its `uploaded-files.tar.gz.dnrenc` companion is also required for recovery onto a fresh server. Decrypt that archive with the same crypto command and password, verify the recorded encrypted checksum, and extract its regular files into a fresh private `uploaded_files` volume owned by `www-data` (files mode `600`, directory mode `700`). Mount that volume into the matching application, backup, and maintenance services before reopening traffic. Retain the existing file volume unchanged during a SQL-only rollback. Do not treat a native SQL archive alone as a complete backup of a release using persistent storage.
